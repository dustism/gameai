...
class SARS:
    ...
    def __init__(self, history, data_queue, camp, hero_i):
    ...
    def end(self, obs):
        self.s1 = obs.extract_feature(self.camp, self.hero_i, SOLDIERS_CONSIDER)
        ...
        if self.s0 is not None:
            ...
            self.r = obs.shape_reward(self.obs0, self.camp, self.hero_i, winner)
...
class ScorePlotter:
    def __init__(self):
        ...
        self.fig = plt.figure()
        self.x_upperlim = 20
        self.ax = plt.axes(xlim=(0, self.x_upperlim), ylim=(-20, 20))
        self.ax.grid()
